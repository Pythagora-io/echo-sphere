...
socket.on('receiveMessage', ({ chatId, senderId, message }) => {
  ...
  if (chatId === currentChatId) { // Check if the received message belongs to the selected chat
    ...
    if (senderId === userId) {
      ...
      messageElement.innerHTML = `<div class="bg-blue-500 float-right clear-right text-right mr-2 p-2 rounded-lg text-white">${message}</div>`;
    } else {
      messageElement.classList.add('items-start');
      messageElement.innerHTML = `<div class="bg-gray-300 dark:bg-gray-600 float-left clear-left text-left ml-2 p-2 rounded-lg text-gray-900 dark:text-gray-300">${message}</div>`;
    }
