    ...
    fetch('/search/trending')
      ...
      .then(data => {
        if (data.success) {
          ...
          mainPanel.innerHTML = '<h1 class="text-4xl font-bold mb-6">Trending Posts</h1>';
          if (data.posts.length > 0) {
            data.posts.forEach(post => {
              ...
              postElement.innerHTML = `
                <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                  <p class="text-blue-500 hover:text-blue-700 font-bold">${post.content}</p>
            ...
            mainPanel.innerHTML += '<p class="text-lg">Explore new topics and engage with the community to discover trending content!</p>';
          }
        } else {
    ...
    fetch('/search/recommendations')
      ...
      .then(data => {
        if (data.success) {
          ...
          const mainPanel = document.getElementById('mainPanel');
          mainPanel.innerHTML = '<h1 class="text-4xl font-bold mb-6">Recommended SubSpheres</h1>';
          if (data.subSpheres.length > 0) {
            ...
            mainPanel.innerHTML += '<p class="text-lg">Keep exploring and engaging with different SubSpheres to receive personalized recommendations!</p>';
          }
        } else {
